...
try:
    from scripts.result import OperationResult, success, failure, default_category
    from scripts.runtime import set_sdk_logging
    from scripts.config import SkillConfig, load_config, validate_config
    from scripts.errors import ConfigError
    from scripts.formatters import set_disclaimer_region
    from scripts.audit import AuditLogger
except ImportError:
    from result import OperationResult, success, failure, default_category
    from runtime import set_sdk_logging
    from config import SkillConfig, load_config, validate_config
    from errors import ConfigError
    from formatters import set_disclaimer_region
...
def dispatch_trading(args: argparse.Namespace, config: SkillConfig) -> str | OperationResult:
    ...
    try:
        ...
        from scripts.trading.account import get_account_list
        from scripts.trading.assets import get_account_balance, get_account_positions
        from scripts.trading.instrument import (
            get_instruments, get_crypto_instruments,
            get_futures_products, get_futures_instruments,
            get_futures_instruments_by_code,
            get_event_series, get_event_instruments,
            get_event_categories, get_event_events,
        )
        from scripts.trading.order import cancel_order, get_order_history, get_open_orders, get_order_detail
        from scripts.trading.stock_order import (
            place_stock_order, preview_stock_order, replace_stock_order,
            place_stock_combo_order, place_algo_order,
        )
        from scripts.trading.option_order import (
            place_option_single_order, preview_option_order, replace_option_order,
            place_option_strategy_order,
        )
        from scripts.trading.futures_order import place_futures_order, replace_futures_order
        from scripts.trading.crypto_order import place_crypto_order
        from scripts.trading.event_order import place_event_order, replace_event_order
    except ImportError:
        from sdk_client import SDKClient
        from risk_engine import RiskEngine
        from trading.account import get_account_list
        from trading.assets import get_account_balance, get_account_positions
        ...
        from trading.order import cancel_order, get_order_history, get_open_orders, get_order_detail
        from trading.stock_order import (
            place_stock_order, preview_stock_order, replace_stock_order,
            place_stock_combo_order, place_algo_order,
        )
        from trading.option_order import (
            place_option_single_order, preview_option_order, replace_option_order,
            place_option_strategy_order,
        )
        from trading.futures_order import place_futures_order, replace_futures_order
        from trading.crypto_order import place_crypto_order
        from trading.event_order import place_event_order, replace_event_order
    ...
    if action == "preview":
        ...
        o = orders[0]
        return _wrap_tool_result(
            preview_stock_order(sdk, aid, **o), action
        )
    if action == "replace":
        if not aid:
            return failure(detail="--account-id required")
        ...
        o = orders[0]
        return _wrap_tool_result(
            replace_stock_order(sdk, aid, **o), action
        )
...
def dispatch_market_data(args: argparse.Namespace, config: SkillConfig) -> str | OperationResult:
    ...
    try:
        from scripts.sdk_client import SDKClient
        from scripts.market_data.stock import (
            get_stock_snapshot, get_stock_bars, get_stock_bars_single,
            get_stock_tick, get_stock_quotes, get_stock_footprint,
        )
        from scripts.market_data.futures import (
            get_futures_snapshot, get_futures_bars, get_futures_tick,
            get_futures_depth, get_futures_footprint,
        )
        from scripts.market_data.crypto import get_crypto_snapshot, get_crypto_bars
        from scripts.market_data.event import (
            get_event_snapshot, get_event_depth, get_event_bars, get_event_tick,
        )
    except ImportError:
        from sdk_client import SDKClient
        from market_data.stock import (
            get_stock_snapshot, get_stock_bars, get_stock_bars_single,
            get_stock_tick, get_stock_quotes, get_stock_footprint,
        )
        ...
        from market_data.crypto import get_crypto_snapshot, get_crypto_bars
        from market_data.event import (
            get_event_snapshot, get_event_depth, get_event_bars, get_event_tick,
        )
...
def dispatch_auth(config: SkillConfig) -> int:
    ...
    """
    try:
        from scripts.sdk_client import SDKClient, TwoFactorAuthRequiredError, DeviceNotRegisteredError
    except ImportError:
        from sdk_client import SDKClient, TwoFactorAuthRequiredError, DeviceNotRegisteredError
