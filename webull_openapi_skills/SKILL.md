---
name: webull-openapi
description: Trade stocks, options, futures, crypto, and event contracts on Webull. Query real-time and historical market data. Manage accounts and positions. Supports US and HK regions with configurable risk controls.
---
...
# Get account balance
python3 scripts/cli.py trading --action balance --account-id <id>
...
#   python3 scripts/cli.py trading --action place --account-id <id> --order-file /tmp/order.json
#
# Alternative (inline, may fail on some shells):
python3 scripts/cli.py trading --action place --account-id <id> \
...
# Cancel an order
python3 scripts/cli.py trading --action cancel --account-id <id> --client-order-id <oid>
```

## CLI Entry Point

```
python3 scripts/cli.py [--env-file PATH] [--verbose-sdk-log] <module> --action <ACTION> [options]
```
...
| `support_trading_session` | US stocks | string | `CORE` (regular hours), `ALL` (extended hours), `NIGHT` (night session) |
...
python3 scripts/cli.py trading --action replace --account-id <id> --order-file /tmp/replace.json
```
...
### Trading Sessions (US stocks only)
...
python3 scripts/cli.py market-data --action stock-snapshot --symbols 00700 --category HK_STOCK

# China Connect A-share
python3 scripts/cli.py market-data --action stock-snapshot --symbols 600519 --category CN_STOCK
```
